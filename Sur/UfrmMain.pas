...
VAR
  ...
  name,SEX,age,KB,SJYS,BCH,BRID,ZDJL,JYYS:string;
begin
  ...
  adotemp.Close;
  adotemp.SQL.Clear;
  adotemp.SQL.Text:='select * from JZXX where RQ='''+FormatDateTime('YYYY-MM-DD',DateTimePicker1.Date)+''' ';
  adotemp.Open;
  while not adotemp.Eof do
  begin
    ID:=adotemp.fieldbyname('ID').AsInteger;
    SXH:=adotemp.fieldbyname('SXH').AsInteger;
    YS:=adotemp.fieldbyname('YS').AsString;//颜色
    TMD:=adotemp.fieldbyname('TMD').AsString;//浊度

    name:=adotemp.fieldbyname('name').AsString;
    SEX:=adotemp.fieldbyname('SEX').AsString;
    age:=adotemp.fieldbyname('age').AsString;
    KB:=adotemp.fieldbyname('KB').AsString;
    SJYS:=adotemp.fieldbyname('SJYS').AsString;
    BCH:=adotemp.fieldbyname('BCH').AsString;
    BRID:=adotemp.fieldbyname('BRID').AsString;
    ZDJL:=adotemp.fieldbyname('ZDJL').AsString;
    JYYS:=adotemp.fieldbyname('JYYS').AsString;

    adotemp22:=tadoquery.Create(nil);
    adotemp22.Connection:=ADOConn_BS;
    adotemp22.Close;
    adotemp22.SQL.Clear;
    adotemp22.SQL.Text:='select * from JZ_ITEM '+
                        ' where ID='+inttostr(ID);
    adotemp22.Open;
    ReceiveItemInfo:=VarArrayCreate([0,adotemp22.RecordCount-1],varVariant);
    i:=0;
    while not adotemp22.Eof do
    begin
      ItemID:=adotemp22.fieldbyname('CAPTION').AsString;
      sValue:=adotemp22.fieldbyname('VAL').AsString;
